...
class ModelRepositoryTest {
    ...
    void shouldCreateNew() throws SQLException {
        ...
        Model modelToSave = new Model(2020, make);
    ...
    @Test
    @Sql(scripts = { "/sql/clear_tables.sql", "/sql/model_test_data.sql" })
    void shoudlFindModelsByCatgiryId() throws SQLException {
        Category category = new Category(100L, "Sedan");
        Make make = new Make(100L, "Audi");
        List<Model> expected = Arrays.asList(new Model(100L, 2020, make));
        List<Model> actual = modelRepository.findModelsByCategories(category.getId());
        assertEquals(expected, actual);
    }
}
