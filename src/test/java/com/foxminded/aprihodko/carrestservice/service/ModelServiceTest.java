...
class ModelServiceTest {
	...
	void shouldFindAllBySearchRequest() throws SQLException {
		ModelSearchRequest modelSearchRequest = new ModelSearchRequest();
		SearchRequest searchRequest = modelSearchRequest.asSerRequest();
		Page<Model> expected = new PageImpl<>(List.of(new Model(100L, "test", new Make(100L, "test1"))));
		when(modelRepository.findAll(any(Specification.class), any(Pageable.class))).thenReturn(expected);
		Page<Model> actual = modelService.findAllBySearchRequest(searchRequest);
